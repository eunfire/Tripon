...
public class LocalBoardRepository {
    ...
    public List<BoardDTO> getBoardsByCateIdWithPaging(int cateId, int startIndex, int pageSize) {
        ...
        return sql.selectList("local.getBoardsByCateIdWithPaging", params);
    }

    // cateId가 1인 게시글 목록 조회
    public int getBoardCountByCateId(int cateId) {
        return sql.selectOne("local.getBoardCountByCateId", cateId);
    }

    // 게시글 상세페이지 조회
    public BoardDTO getBoardById(int postId) {
        return sql.selectOne("local.getBoardById", postId);
    }

    // 게시글 조회수 증가
    public void increaseViews(int postId) {
        sql.update("local.increaseViews", postId);
    }
}